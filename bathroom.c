...
void updateOverallmin(Thebathroom *CommonBathroom, long qtime) {
    if (CommonBathroom->Sim_min_q_time > qtime) {
        CommonBathroom->Sim_min_q_time = qtime;
...
void sumQtime(Thebathroom *CommonBathroom, int qtime) {
    ...
    CommonBathroom->Sim_total_q_time += qtime;
}
//updatemaxtime
void updateOverallmax(Thebathroom *CommonBathroom, int qtime) {
    if (CommonBathroom->Sim_max_q_time < qtime) {
        CommonBathroom->Sim_max_q_time = qtime;
...
void Initialize(Thebathroom *CommonBathroom) {
    ...
    CommonBathroom->Sim_total_inQ = 0; //number of people who were in the q through the sim
    CommonBathroom->Sim_total_q_time = 0; //overall total q time (sum of all people's q time)
    CommonBathroom->Sim_min_q_time = 0; //overall min q time of all people
    CommonBathroom->Sim_max_q_time = 0; //overall max q time of all people
    CommonBathroom->Sim_avg_q_time = 0; //Overall_total_q_time/Sim_total_inQ = overall avg q time
    CommonBathroom->total_uses = 0; //total uses the bathroom has gone through
    CommonBathroom->Sim_avg_users = 0;
...
//change state of bathroom
        ...
        CommonBathroom->Sim_total_inQ++; //increment total waitq
...
void Finalize(Thebathroom *CommonBathroom) {
    ...
    //calc avg q time
    CommonBathroom->Sim_avg_q_time =
            (float) CommonBathroom->Sim_total_q_time / (float) CommonBathroom->Sim_total_inQ;
    //calc avg users per milisecond
    CommonBathroom->Sim_avg_users = (float) CommonBathroom->total_uses / (float) CommonBathroom->timediff;
